...
import {
  BookOpen, Briefcase, DollarSign, LogOut, PanelLeft, Settings, User,
} from "lucide-react";
        ...
        <SidebarMenu className="app-sidebar__nav-menu">
          {currentNavLinks.map((link) => {
            ...
            return (
              <SidebarMenuItem
                key={link.href}
                className={cn( "app-sidebar__nav-item", isActive && "bg-primary-900")}>
                <SidebarMenuButton asChild size="lg" className={cn("app-sidebar__nav-button", !isActive && "text-customTeals-mutedTeal")}>
                  <Link href={link.href} className="app-sidebar__nav-link" scroll={false}  >
                    <link.icon className={isActive ? "text-white-100" : "text-teal-500"} />
