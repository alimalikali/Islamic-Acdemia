      ...
      case "select":
        return (
          ...
          >
            ...
            <SelectContent className="w-full bg-customTeals-primarybg border-customTeals-mutedTeal shadow">
              {options?.map((option) => (
                <SelectItem
                  key={option.value}
                  value={option.value}
        ...
        return (
          <div className="flex items-center space-x-2">
            <Switch
              ...
              id={name}
              className={`text-customTeals-mutedTeal ${inputClassName}`}
            />
        ...
        >
          {type !== "switch" && (
            <div className="flex justify-between items-center">
              <FormLabel
                className={`text-customTeals-mutedTeal text-sm ${labelClassName}`}
              >
                {label}
              </FormLabel>
              ...
              {!disabled &&
                ...
                type !== "file" &&
                type !== "multi-input" && (
                  <Edit className="size-4 text-customTeals-mutedTeal" />
                )}
            </div>
          )}
          <FormControl>
  ...
  return (
    <div className="space-y-2">
      {fields.map((field, index) => (
        <div key={field.id} className="flex items-center space-x-2">
            ...
            render={({ field }) => (
              <FormControl>
                <Input
                  {...field}
                  placeholder={placeholder}
                  className={`flex-1 border-none bg-customTeals-darkTeal p-4 ${inputClassName}`}
                />
              </FormControl>
            )}
          />
          <Button
            ...
            variant="ghost"
            size="icon"
            className="text-customTeals-mutedTeal"
          >
            <X className="w-4 h-4" />
          </Button>
        </div>
      ...
      <Button
        ...
        variant="outline"
        size="sm"
        className="mt-2 text-customTeals-mutedTeal"
      >
